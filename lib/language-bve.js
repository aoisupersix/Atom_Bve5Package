...
import SearchCommentView from './bve-searchCommentView.js';
...
export default {
  ...
  commentSearchListView: null, //コメント検索ビュー
  subscriptions: null,
  ...
  activate(state) {
    ...
    this.commentSearchListView = new SearchCommentView(state.languageBveViewState);
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'language-bve:JumpComment': () => this.openSearchCommentPanel(),
  ...
  //コメント検索パネルを表示
  openSearchCommentPanel(){
    if(this.commentSearchListView.isVisible){
      this.commentSearchListView.hide();
    }else{
      this.commentSearchListView.show(atom.workspace.getActiveTextEditor());
    };
  },
